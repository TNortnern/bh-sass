/**
 * Catch-all proxy for /v1/* routes to Payload CMS /api/*
 * This forwards requests like /v1/users/me to /api/users/me on Payload
 */
...
export default defineEventHandler(async (event) => {
  ...
  // Get the path after /v1/ and map to /api/
  const v1Path = event.path.replace(/^\/v1/, '')
  const targetUrl = `${payloadUrl}/api${v1Path}`
  ...
  try {
    ...
    // Forward Set-Cookie headers from Payload for authentication
