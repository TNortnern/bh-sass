<template>
  <section ref="sectionRef" class="py-20 lg:py-32 bg-gray-50 overflow-hidden">
          ...
          <div class="relative">
            ...
            <div class="bg-white rounded-2xl shadow-2xl border border-gray-200 p-6 lg:p-8">
              ...
              <div class="flex items-center justify-between mb-6">
                <div>
                  <h3 class="text-2xl font-bold text-gray-900">June 2025</h3>
                  <p class="text-sm text-gray-500">23 bookings this month</p>
                </div>
                <div class="flex gap-2">
                  <button class="w-10 h-10 rounded-lg border border-gray-200 flex items-center justify-center hover:bg-gray-50 transition-colors">
                    <UIcon name="i-lucide-chevron-left" class="text-gray-600" />
                  </button>
                  <button class="w-10 h-10 rounded-lg border border-gray-200 flex items-center justify-center hover:bg-gray-50 transition-colors">
                    <UIcon name="i-lucide-chevron-right" class="text-gray-600" />
                  </button>
                </div>
              </div>
              ...
              <div class="grid grid-cols-7 gap-2 mb-2">
                <!-- Day Headers -->
                <div v-for="day in ['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun']" :key="day" class="text-center text-sm font-semibold text-gray-600 py-2">
                  {{ day }}
                </div>
                ...
                <!-- Calendar Days -->
                <div v-for="date in 28" :key="date" class="aspect-square rounded-lg border border-gray-200 p-1 hover:border-orange-300 transition-colors cursor-pointer relative">
                  <span class="text-sm font-medium text-gray-700">{{ date }}</span>

                  <!-- Booking indicators -->
                  <div v-if="[5, 8, 12, 15, 19, 22, 26].includes(date)" class="absolute bottom-1 left-1 right-1 flex gap-0.5">
                    <div class="flex-1 h-1 rounded-full bg-blue-400"></div>
                  </div>
                  <div v-if="[8, 15, 22].includes(date)" class="absolute bottom-1 left-1 right-1 flex gap-0.5">
                    <div class="flex-1 h-1 rounded-full bg-blue-400"></div>
                    <div class="flex-1 h-1 rounded-full bg-purple-400"></div>
                  </div>
                  <div v-if="date === 12" class="absolute inset-0 bg-orange-500/10 rounded-lg border-2 border-orange-500">
              ...
              <div class="mt-6 bg-gradient-to-br from-gray-50 to-gray-100 rounded-xl p-4 border border-gray-200">
                <h4 class="text-sm font-semibold text-gray-900 mb-3 flex items-center gap-2">
                  <UIcon name="i-lucide-truck" class="text-orange-500" />
                  Today's Deliveries
                </h4>
                <div class="space-y-3">
                  <div class="bg-white rounded-lg p-3 border border-gray-200">
                    ...
                    <p class="text-sm text-gray-600">Princess Castle → 123 Party Lane</p>
                    <p class="text-xs text-gray-500 mt-1">Sarah Johnson • (555) 123-4567</p>
                  </div>
                  <div class="bg-white rounded-lg p-3 border border-gray-200">
                    <div class="flex items-center justify-between mb-1">
                    ...
                    <p class="text-sm text-gray-600">Water Slide XL → 456 Fun Street</p>
                    <p class="text-xs text-gray-500 mt-1">Mike Davis • (555) 987-6543</p>
                  </div>
                </div>
              </div>
            </div>
