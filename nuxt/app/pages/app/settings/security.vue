<template>
  <div class="max-w-[1200px] mx-auto">
    ...
    <div class="mb-8 pb-6 border-b border-gray-200 dark:border-white/[0.06]">
      <div>
        <h2 class="text-2xl font-bold tracking-tight m-0 mb-1.5 text-gray-900 dark:text-white">Security & Activity</h2>
        <p class="m-0 text-[0.9375rem] text-gray-600 dark:text-[#888]">Monitor account activity and security settings</p>
      </div>
    </div>
      ...
      <UCard class="bg-white dark:bg-white/[0.02] border border-gray-200 dark:border-white/[0.06] rounded-2xl overflow-hidden transition-all duration-300 hover:border-amber-300 dark:hover:border-amber-500/20 hover:shadow-lg hover:shadow-amber-500/10">
        <template #header>
          <div class="flex items-center gap-4">
            ...
            <div>
              <h3 class="text-lg font-semibold tracking-tight m-0 mb-1 text-gray-900 dark:text-white">Active Sessions</h3>
        ...
        <div class="p-6">
          <div v-if="sessions.length === 0" class="flex flex-col items-center justify-center p-8 gap-3">
            <UIcon name="i-lucide-monitor" class="w-10 h-10 text-gray-300 dark:text-[#333]" />
            <p class="m-0 text-[0.9375rem] text-gray-500 dark:text-[#666]">No active sessions</p>
          </div>
          ...
          <div v-if="sessions.length > 1" class="mt-4 pt-4 border-t border-gray-200 dark:border-white/[0.06]">
            <UButton
              variant="outline"
              color="error"
              size="md"
      ...
      <UCard class="bg-white dark:bg-white/[0.02] border border-gray-200 dark:border-white/[0.06] rounded-2xl overflow-hidden transition-all duration-300 hover:border-amber-300 dark:hover:border-amber-500/20 hover:shadow-lg hover:shadow-amber-500/10">
        <template #header>
          <div class="flex items-center gap-4">
            ...
            <div>
              <h3 class="text-lg font-semibold tracking-tight m-0 mb-1 text-gray-900 dark:text-white">Two-Factor Authentication</h3>
              <p class="m-0 text-sm text-gray-500 dark:text-[#666]">
        ...
        <div class="p-6">
          <div class="flex flex-col gap-6">
            ...
            <div v-if="twoFactorEnabled" class="flex items-start gap-3 p-4 bg-blue-50 dark:bg-blue-500/5 border border-blue-200 dark:border-blue-500/15 rounded-lg">
      ...
      <UCard class="bg-white dark:bg-white/[0.02] border border-gray-200 dark:border-white/[0.06] rounded-2xl overflow-hidden transition-all duration-300 hover:border-amber-300 dark:hover:border-amber-500/20 hover:shadow-lg hover:shadow-amber-500/10">
        <template #header>
          <div class="flex items-center gap-4">
            ...
            <div>
              <h3 class="text-lg font-semibold tracking-tight m-0 mb-1 text-gray-900 dark:text-white">Password Security</h3>
              <p class="m-0 text-sm text-gray-500 dark:text-[#666]">Password requirements and best practices</p>
            </div>
          </div>
        </template>
        ...
        <div class="p-6">
          <div class="flex flex-col gap-6">
            <h4 class="text-[0.9375rem] font-semibold text-gray-900 dark:text-white m-0 mb-3">Password Requirements</h4>
            <ul class="m-0 p-0 list-none flex flex-col gap-3">
              <li class="flex items-center gap-3 text-sm text-gray-700 dark:text-[#ccc]">
                <UIcon name="i-lucide-check" class="w-4 h-4 text-green-600 dark:text-green-500 shrink-0" />
      ...
      <UCard class="bg-white dark:bg-white/[0.02] border border-gray-200 dark:border-white/[0.06] rounded-2xl overflow-hidden transition-all duration-300 hover:border-amber-300 dark:hover:border-amber-500/20 hover:shadow-lg hover:shadow-amber-500/10">
        <template #header>
          <div class="flex items-center gap-4">
            ...
            <div>
              <h3 class="text-lg font-semibold tracking-tight m-0 mb-1 text-gray-900 dark:text-white">Login History</h3>
              <p class="m-0 text-sm text-gray-500 dark:text-[#666]">Recent authentication attempts</p>
            </div>
          </div>
        </template>

        <div class="p-6">
          <div v-if="loginHistory.length === 0" class="flex flex-col items-center justify-center p-8 gap-3">
            <UIcon name="i-lucide-history" class="w-10 h-10 text-gray-300 dark:text-[#333]" />
            <p class="m-0 text-[0.9375rem] text-gray-500 dark:text-[#666]">No login history available</p>
          </div>
          ...
          <div v-else class="flex flex-col gap-3">
            ...
            >
              ...
              <div class="flex-1 min-w-0">
                ...
                <div v-if="!login.success && login.reason" class="flex items-center gap-2 mt-2 p-2 bg-red-50 dark:bg-red-500/10 border border-red-200 dark:border-red-500/20 rounded-md text-[0.8125rem] text-red-700 dark:text-red-300">
          ...
          <div v-if="loginHistory.length >= 10" class="mt-4 pt-4 border-t border-gray-200 dark:border-white/[0.06] flex justify-center">
            <UButton
              variant="ghost"
              size="md"
      ...
      <UCard class="bg-white dark:bg-white/[0.02] border border-gray-200 dark:border-white/[0.06] rounded-2xl overflow-hidden transition-all duration-300 hover:border-amber-300 dark:hover:border-amber-500/20 hover:shadow-lg hover:shadow-amber-500/10">
        <template #header>
          <div class="flex items-center gap-4">
            ...
            <div>
              <h3 class="text-lg font-semibold tracking-tight m-0 mb-1 text-gray-900 dark:text-white">Activity Audit Log</h3>
              <p class="m-0 text-sm text-gray-500 dark:text-[#666]">Track all account actions</p>
            </div>
          </div>
        </template>

        <div class="p-6">
          <div v-if="auditLogs.length === 0" class="flex flex-col items-center justify-center p-8 gap-3">
            <UIcon name="i-lucide-file-text" class="w-10 h-10 text-gray-300 dark:text-[#333]" />
            <p class="m-0 text-[0.9375rem] text-gray-500 dark:text-[#666]">No audit logs available</p>
          </div>
            ...
            >
              ...
              <div class="flex-1 min-w-0">
                ...
                <div class="flex flex-wrap gap-4 mt-2">
                  ...
                  <span v-if="log.metadata?.ipAddress" class="flex items-center gap-1.5 text-[0.8125rem] text-gray-600 dark:text-[#888]">
                    <UIcon name="i-lucide-globe" class="w-3.5 h-3.5" />
          ...
          <div v-if="auditLogs.length >= 20" class="mt-4 pt-4 border-t border-gray-200 dark:border-white/[0.06] flex justify-center">
            <UButton
              variant="ghost"
              size="md"
    ...
    <UModal v-model:open="showChangePasswordModal">
      <template #content>
        <UCard>
          <template #header>
            <h3 class="text-xl font-semibold m-0 text-gray-900 dark:text-white">Change Password</h3>
          </template>
          ...
          <template #footer>
            <div class="flex gap-3 justify-end">
              <UButton variant="ghost" @click="showChangePasswordModal = false">
                Cancel
              </UButton>
              <UButton
                color="primary"
...
const loadSecurityData = async () => {
  ...
  try {
    ...
    auditLogs.value = auditData?.docs || []
...
const loadMoreHistory = async () => {
  try {
    ...
    const moreHistory = await $fetch(`/api/security/login-history?offset=${offset}`)
...
const loadMoreAuditLogs = async () => {
  try {
    ...
    const moreLogs = await $fetch(`/api/audit-logs?limit=20&offset=${offset}`)
