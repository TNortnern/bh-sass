      ...
      <UCard class="bg-white dark:bg-white/[0.02] border border-gray-200 dark:border-white/[0.06] rounded-2xl overflow-hidden transition-all duration-300 hover:border-amber-300 dark:hover:border-amber-500/20 hover:shadow-lg dark:hover:shadow-[0_8px_32px_-8px_rgba(251,191,36,0.15)]">
        ...
        <div class="p-6">
          ...
          >
            ...
            />
            <p class="m-0 text-lg font-semibold text-gray-900 dark:text-white">
            ...
            >
              <div class="flex items-start justify-between gap-4">
                <div class="flex-1 min-w-0">
                  ...
                  <p class="text-[0.9375rem] font-semibold text-gray-900 dark:text-white m-0 mb-2">
                    {{ endpoint.name }}
                  </p>
        ...
        <div class="p-6">
          <div class="grid grid-cols-[repeat(auto-fill,minmax(280px,1fr))] gap-4">
            ...
            >
              ...
              <div class="flex-1">
                <h4 class="text-sm font-semibold m-0 mb-1 text-gray-900 dark:text-white font-mono">
                  {{ event.label }}
                </h4>
                <p class="m-0 text-[0.8125rem] text-gray-600 dark:text-[#888] leading-snug">
                  {{ event.description }}
                </p>
              </div>
            </div>
        ...
        <UCard>
          ...
          <div class="p-6 flex flex-col gap-6 max-h-[70vh] overflow-y-auto">
            <UFormField
              label="Name"
              required
              help="A friendly name for this endpoint"
            >
              <UInput
        ...
        <UCard class="max-w-[700px]">
          ...
          <div class="p-6">
            <div
              ...
              class="flex flex-col items-center justify-center py-8 gap-3 text-gray-600 dark:text-[#888]"
            >
              <div class="w-6 h-6 border-2 border-amber-200 dark:border-amber-500/10 border-t-amber-600 dark:border-t-amber-400 rounded-full animate-spin" />
            ...
            >
              ...
              >
                ...
                <div
                  v-if="delivery.error"
                  class="flex items-center gap-2 p-2 bg-red-100 dark:bg-red-500/10 border border-red-200 dark:border-red-500/20 rounded-md text-[0.8125rem] text-red-700 dark:text-red-300"
                >
                  <UIcon
                    name="i-lucide-alert-circle"
                ...
                <div
                  ...
                  class="flex items-center gap-2 p-2 bg-amber-100 dark:bg-amber-500/10 border border-amber-300 dark:border-amber-500/20 rounded-md text-[0.8125rem] text-amber-800 dark:text-amber-200"
                >
                  <UIcon
                    name="i-lucide-clock"
                    class="w-4 h-4 flex-shrink-0"
...
const loadEndpoints = async () => {
  ...
  try {
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const response = await $fetch<{ docs: any[] }>('/api/webhook-endpoints')
...
const viewDeliveries = async (endpointId: string) => {
  ...
  try {
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const response = await $fetch<{ docs: any[] }>(`/api/webhook-deliveries?endpointId=${endpointId}`)
