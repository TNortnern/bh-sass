      ...
      <UCard class="relative bg-gray-900/90 backdrop-blur-xl border-gray-700/50">
        <form @submit.prevent="handleNext" class="space-y-6">
          ...
          <div>
            ...
            >
              <div v-if="!form.photo" class="text-center">
                <div class="inline-flex items-center justify-center w-16 h-16 rounded-full bg-gray-800/50 mb-4 group-hover:bg-amber-500/10 transition-colors">
                  <Icon name="lucide:image" class="w-8 h-8 text-gray-500 group-hover:text-amber-400 transition-colors" />
                </div>
                <p class="text-gray-400 mb-2">Click to upload a photo</p>
          ...
          <!-- Preview Card -->
          <div v-if="form.name || form.description || form.price" class="pt-4">
            <label class="block text-sm font-medium text-gray-300 mb-3">
              Preview
            </label>
