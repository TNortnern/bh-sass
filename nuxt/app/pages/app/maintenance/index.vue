  ...
  <div>
    ...
    <div class="space-y-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
          <UCard>
            <div class="flex items-center justify-between">
              ...
              <UIcon name="i-lucide-alert-triangle" class="text-4xl text-red-600 dark:text-red-400" />
            </div>
          </UCard>
          ...
          <UCard>
            <div class="flex items-center justify-between">
              <div>
                ...
                <p class="text-2xl font-bold text-yellow-600 dark:text-yellow-400">{{ stats.dueSoon }}</p>
              </div>
              <UIcon name="i-lucide-clock" class="text-4xl text-yellow-600 dark:text-yellow-400" />
            </div>
          </UCard>
          ...
          <UCard>
            <div class="flex items-center justify-between">
              <div>
                ...
                <p class="text-2xl font-bold">{{ stats.completedThisMonth }}</p>
              </div>
              <UIcon name="i-lucide-check-circle" class="text-4xl text-green-600 dark:text-green-400" />
            </div>
          </UCard>
          ...
          <UCard>
            <div class="flex items-center justify-between">
              <div>
                ...
                <p class="text-2xl font-bold">${{ stats.totalCost.toFixed(2) }}</p>
              </div>
              <UIcon name="i-lucide-dollar-sign" class="text-4xl text-blue-600 dark:text-blue-400" />
            </div>
          </UCard>
        </div>
        ...
        <div v-if="overdue.length > 0">
          <div class="flex items-center justify-between mb-4">
            <h2 class="text-lg font-semibold flex items-center gap-2">
              <UIcon name="i-lucide-alert-triangle" class="text-red-600 dark:text-red-400" />
              Overdue Maintenance
            </h2>
          </div>

          <div class="space-y-3">
            <UCard v-for="record in overdue" :key="record.id" class="border-l-4 border-red-500">
              <div class="flex items-center justify-between">
                <div class="flex-1">
                  <div class="flex items-center gap-3 mb-2">
                    <UIcon :name="getTypeIcon(record.type)" class="text-gray-400" />
                    <h3 class="font-medium">{{ record.description }}</h3>
                    <UBadge :label="record.type" color="neutral" variant="subtle" />
                  </div>
        ...
        <div v-if="dueSoon.length > 0">
          <div class="flex items-center justify-between mb-4">
            <h2 class="text-lg font-semibold flex items-center gap-2">
              <UIcon name="i-lucide-clock" class="text-yellow-600 dark:text-yellow-400" />
              Due Soon
            </h2>
          </div>

          <div class="space-y-3">
            <UCard v-for="record in dueSoon" :key="record.id" :class="[
              'border-l-4',
